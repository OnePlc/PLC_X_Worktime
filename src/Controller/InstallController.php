    ...
    public function checkdbAction()
    {
        ...
        if(! $oRequest->isPost()) {
            ...
            return new ViewModel([
                ...
                'sVendor' => 'oneplace',
                'sModule' => 'oneplace-worktime',
            ]);
        } else {
            $sSetupConfig = $oRequest->getPost('plc_module_setup_config');

            $sSetupFile = 'vendor/oneplace/oneplace-worktime/data/install.sql';
            ...
            if($sSetupConfig != '') {
                $sConfigStruct = 'vendor/oneplace/oneplace-worktime/data/structure_'.$sSetupConfig.'.sql';
                ...
                $sConfigData = 'vendor/oneplace/oneplace-worktime/data/data_'.$sSetupConfig.'.sql';
                ...
                if(file_exists($sCustomLangFile)) {
                    ...
                    copy($sCustomLangFile,'vendor/oneplace/oneplace-worktime/language/de_DE.po');
                    $sCustomLangFile = 'vendor/oneplace/oneplace-worktime/data/de_DE_'.$sSetupConfig.'.mo';
                    copy($sCustomLangFile,'vendor/oneplace/oneplace-worktime/language/de_DE.mo');
                    echo $sCustomLangFile. " to ".'vendor/oneplace/oneplace-worktime/language/de_DE.mo';
                }
            }

            $oModTbl = new TableGateway('core_module', CoreUpdateController::$oDbAdapter);
            $oModTbl->insert([
                'module_key'=>'oneplace-worktime',
                'type'=>'module',
                'version'=>\OnePlace\Worktime\Module::VERSION,
                'label'=>'onePlace Worktime',
                'vendor'=>'oneplace',
            ]);
            ...
            $this->flashMessenger()->addSuccessMessage('Worktime DB Update successful');
